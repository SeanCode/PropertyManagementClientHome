...
<template>
  <div class="mz-datepicker" :style="{width:width+'px'}">
    <input :value="value" placeholder="{{placeholder}}" readonly :disabled="disabled" :class="{focus:show}"
           @click="click"
           @mousedown="$event.preventDefault()"/><a v-if="clearable&&value" @click.stop="clear"></a><i></i>
    <div class="mz-datepicker-popup" :class="{'mz-datepicker-popup-left':left}" v-if="show"
         transition="mz-datepicker-popup" tabindex="-1" @blur="show = false" @mousedown="$event.preventDefault()"
         @keyup.up="changeMonth(-1,1)" @keyup.down="changeMonth(1,1)" @keyup.left="changeYear(-1,1)"
         @keyup.right="changeYear(1,1)" v-el:popup>
        ...
        <template v-for="no in count">
          <div class="mz-calendar">
            ...
            <table cellspacing="0" cellpadding="0">
              ...
              <tr v-if="this['date'+(no+1)]" v-for="i in 6">
                <td v-for="j in 7" v-text="this['date'+(no+1)][i * 7 + j].text"
                    :title="this['date'+(no+1)][i * 7 + j].title" :class="this['date'+(no+1)][i * 7 + j].status"
                    @click="select(this['date'+(no+1)][i * 7 + j], no+1)"></td>
              </tr>
            </table>
            <div class="mz-calendar-year-panel" transition="mz-calendar-panel" v-if="this['showYear'+(no+1)]"><a
              class="mz-calendar-year-panel-prev" @click="changeYearRange(no+1,-1)"></a><a
              class="mz-calendar-year-panel-year" v-for="item in this['years'+(no+1)]" :class="item.status"
              @click="selectYear(item,no+1)">{{item.year}}年</a><a class="mz-calendar-year-panel-next"
                                                                  @click="changeYearRange(no+1,1)"></a></div>
            <div class="mz-calendar-month-panel" transition="mz-calendar-panel" v-if="this['showMonth'+(no+1)]"><a
              v-for="item in this['months'+(no+1)]" class="mz-calendar-month-panel-month" :class="item.status"
              @click="selectMonth(item,no+1)">{{item.month}}月</a></div>
          </div>
  ...
  export default {
    ...
    computed: {
      ...
      placeholder: function () {
        return new Date(this.time * 1000).toLocaleDateString()
      }
    },
    watch: {
      show: function (val) {
    ...
    methods: {
      ...
      click: function () {
        ...
        var tempRight = self.range ? 441 : 214
        if (right < tempRight && right < rect.left) {
          self.left = true
        } else {
          self.left = false
        }
      ...
      showYear: function (no) {
        ...
        var num = Math.floor(time.getFullYear() % 10) // 获取当前时间个位数
        var arr = []
